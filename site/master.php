...
class Master extends Eden {
	...
	public function setPaths(array $paths = array()) {
		$default = array(
			...
			'config'		  => $this->_root.'/master/config',
			'template'		=> $this->_root.'/master/template',
	...
	public function setRequest(Eden_Registry $request = NULL) {
		...
		if(!empty($suggestions)) {
			...
			while(count($classBuffer) > 1) {
				...
				$page = 'Master_Page'.str_replace(' ', '_', $classParts);
	...
	public function addDatabase($key, 			$type = NULL, 
		...
		switch($type) {
			case 'postgre':
				...
				break;
			case 'mysql':
				$database = Master_Mysql::i($host, $name, $user, $pass); 
				break;
		}
	...
	public function setDefaultDatabase($key) {
		Master_Error::i()->argument(1, 'string');
	...
	public function setFilters($filters) {
		Master_Error::i()->argument(1, 'string', 'array');
	...
	public function block($name) {
		...
		$class 	= 'Master_Block_'.ucwords($name);
	...
	public function error($e, $event, $type, $level, $class, $file, $line, $message, $trace, $offset) {
		...
		echo Eden_Template::i()
			...
			->set('line', $line)
			->set('message', $message)
			->parsePhp(dirname(__FILE__).'/master/template/error.php');
	}
